...
class BootReceiverTest {
    ...
    fun launchesWhenBootCompletedAndAutostartEnabled() = runTest {
        ...
        BootReceiver.handle(Intent.ACTION_BOOT_COMPLETED, autostartEnabled = true, launcher = launcher)
    ...
    fun doesNotLaunchWhenAutostartDisabled() = runTest {
        val launcher = FakeLauncher()
        BootReceiver.handle(Intent.ACTION_BOOT_COMPLETED, autostartEnabled = false, launcher = launcher)
    ...
    fun ignoresUnrelatedActions() = runTest {
        val launcher = FakeLauncher()
        BootReceiver.handle(Intent.ACTION_BATTERY_LOW, autostartEnabled = true, launcher = launcher)
        assertThat(launcher.launched).isEqualTo(0)
    }
    ...
    fun acceptsQuickbootPowerOn() = runTest {
        val launcher = FakeLauncher()
        BootReceiver.handle("android.intent.action.QUICKBOOT_POWERON", autostartEnabled = true, launcher = launcher)
        assertThat(launcher.launched).isEqualTo(1)
    }
}
